function [A handles] = TestRun(S,P,A,hObject)
...
%         end
        ...
        if currentTime > lastFrameTime + frameTimeStep - 0.006
            A.startTime(end+1)=currentTime;
            if showStim
                % Update frame
                frameI = frameI + 1;
                
                %draw flickering grating
                if mod(frameI,P.tF)==0
                    cID=~cID;
                end
                
                
                Screen('DrawTexture',A.window,A.gratTex(cID+1),[],A.gratPos,0);
                Screen('DrawingFinished',A.window);
            ...
            % FLIP THE FRAME
            % if currentTime > lastFrameTime + frameTimeStep - 0.006
            lastFrameTime = Screen('Flip',A.window);
            A.frameTime(end+1)=lastFrameTime;
            
            
            %             % Pause and update handles
            
               pause(.0005); handles = guidata(hObject);
              A.remTime(end+1)=GetSecs; 
        end
        
        %if  currentTime > lastFrameTime + frameTimeStep - 0.001
