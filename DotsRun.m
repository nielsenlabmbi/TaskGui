...
while state < 5 && ~handles.abortTask
    ...
    % CONTROL STIMULUS PRESENTATION DURATION
    if state == 1 || state == 2
        if currentTime > A.choiceTime + P.durStim
            showStim = 0;
        end
    end
    ...
    if currentTime > lastFrameTime + frameTimeStep - 0.006
        ...
        if showStim
            ...
            frameI=frameI+1;
            if frameI<=A.nrFrames
                
                Screen('DrawDots', A.window, A.dotpos{frameI}, A.dotSize, A.dotcolor, ...
                [A.screenRect(3)/2 A.screenRect(4)/2],1);  
               %Screen('DrawTexture',A.window,A.frame(frameI));
            end
        end
