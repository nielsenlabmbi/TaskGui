function [A, handles] = SearchRun(S,P,A,hObject)
...
while state < 5 && ~handles.abortTask
    ...
    if currentTime > lastFrameTime + frameTimeStep - 0.006
        ...
        if showStim 
            if A.targetType==1
                Screen ('FillPoly',A.window,A.colorTarget,A.trianglePos{1},0);
%               
                Screen ('DrawDots',A.window,A.dotPos',2*A.dotRadius,A.colorDist,...
