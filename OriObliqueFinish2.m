function D = OriObliqueFinish2(D,A)
...
D.numR53 = sum(D.angle == 127);
D.numR37 = sum(D.angle == 143);
...
D.ncR53 = sum(D.correct & (D.angle == 127));
D.fcR53 = D.ncR53/D.numR53;
D.ncL53 = sum(D.correct & (D.angle == 53));
D.fcL53 = D.ncL53/D.numL53;

D.ncR37 = sum(D.correct & (D.angle == 143));
D.fcR37 = D.ncR37/D.numR37;
...
% end
% 
% if ~isnan(D.side(D.angle == 143))
%     bar(10,100*D.fcR37,'FaceColor',colors(mean(D.side(D.angle==143)),:));    %plots percentage of left data correct
% else bar(10,100*D.fcR37,'FaceColor',[1 0 0]);    %plots percentage of left data correct
% end
% if ~isnan(D.side(D.angle == 127))
% bar(11,100*D.fcR53,'FaceColor',colors(mean(D.side(D.angle==127)),:));    %plots percentage of left data correct
% else bar(11,100*D.fcR53,'FaceColor',[1 0 0]);    %plots percentage of left data correct
% end
...
if ~isnan(D.side(D.angle == 37))
    bar(10,100*D.fcL37,'FaceColor',colors(mean(D.side(D.angle==37)),:));    %plots percentage of left data correct
else bar(10,100*D.fcL37,'FaceColor',[1 0 0]);    %plots percentage of left data correct
end
if ~isnan(D.side(D.angle == 53))
bar(11,100*D.fcL53,'FaceColor',colors(mean(D.side(D.angle==53)),:));    %plots percentage of left data correct
else bar(11,100*D.fcL53,'FaceColor',[1 0 0]);    %plots percentage of left data correct
end
...
% set(gca(1),'XTickLabel', {'All' 'L(-20)' 'R(+20)' 'L(-17)' 'R(+17)' 'L(-14)' 'R(+14)' 'L(-11)' 'R(+11)' 'L(-8)' 'R(+8)'}); %Then labels the three bars.
set(gca(1),'XTickLabel', {'All' 'R(-20)' 'L(+20)' 'R(-17)' 'L(+17)' 'R(-14)' 'L(+14)' 'R(-11)' 'L(+11)' 'R(-8)' 'L(+8)'}); %Then labels the three bars.
...
% rndPercentR37=strcat({num2str(round(100*D.fcR37))},'%');
% rndPercentR53=strcat({num2str(round(100*D.fcR53))},'%');
rndPercentLR=strcat({num2str(round(100*D.fcAll))},'%'); 
...
% y1=[rndPercentLR rndPercentR25 rndPercentR65 rndPercentR28 rndPercentR62 rndPercentR31 rndPercentR59 rndPercentR34 rndPercentR56 rndPercentR37 rndPercentR53]; %Used as the percentage text being displayed on graph
y1=[rndPercentLR rndPercentL25 rndPercentL65 rndPercentL28 rndPercentL62 rndPercentL31 rndPercentL59 rndPercentL34 rndPercentL56 rndPercentL37 rndPercentL53]; %Used as the percentage text being displayed on graph
...
% rndTotalR37=strcat('N=',{num2str(D.numR37)});
% rndTotalR53=strcat('N=',{num2str(D.numR53)});
rndTotalLR=strcat('N=',{num2str(D.numTrials)});
rndTotalL25=strcat('N=',{num2str(D.numL25)});
rndTotalL65=strcat('N=',{num2str(D.numL65)});
rndTotalL28=strcat('N=',{num2str(D.numL28)});
rndTotalL62=strcat('N=',{num2str(D.numL62)});
rndTotalL31=strcat('N=',{num2str(D.numL31)});
rndTotalL59=strcat('N=',{num2str(D.numL59)});
rndTotalL34=strcat('N=',{num2str(D.numL34)});
rndTotalL56=strcat('N=',{num2str(D.numL56)});
rndTotalL37=strcat('N=',{num2str(D.numL37)});
rndTotalL53=strcat('N=',{num2str(D.numL53)});

% y2=[rndTotalLR rndTotalR25 rndTotalR65 rndTotalR28 rndTotalR62 rndTotalR31 rndTotalR59 rndTotalR34 rndTotalR56 rndTotalR37 rndTotalR53]; %Used as the total text being displayed on graph \
y2=[rndTotalLR rndTotalL25 rndTotalL65 rndTotalL28 rndTotalL62 rndTotalL31 rndTotalL59 rndTotalL34 rndTotalL56 rndTotalL37 rndTotalL53]; %Used as the total text being displayed on graph \
