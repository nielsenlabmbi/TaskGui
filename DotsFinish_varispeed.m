function D = DotsFinish_varispeed(D,A)
...
for i=1:length(speeds)
    idxR = find(D.dotSpeed==speeds(i) & D.side==1);
    idxL = find(D.dotSpeed==speeds(i) & D.side==2);
...
%AXIS SETTIGNS
axis([.5 length(speeds)*2+1.5 0 110.5]); %sets the x axis min and max and y axis min and max
%X AXIS
set(gca(1),'XTick',[1:2*length(speeds)+1]) %Changes the graph to have three x axis labels instead of the default 1
set(gca(1),'XTickLabel', {'All Trials' 'L1.2' 'R1.2' 'L.6' 'R.6' 'L.4' 'R.4' 'L.2' 'R.2' 'L.05' 'R.05'}); 
%Y AXIS
