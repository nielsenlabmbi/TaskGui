function [A handles] = PlaidRun(S,P,A,hObject)

% function A = PlaidRun(S,P,A)
%
...
while state < 5 && ~handles.abortTask
    ...
    %if currentTime > lastFrameTime + 2*frameTimeStep - 0.008
        ...
        if showStim 
            % Update frame
            frameI = frameI + 1;
            Screen('DrawTexture',A.window,A.gratTex(A.side,A.plaid+1).a(frameI),[],A.gratPos,0);
           
            %Mod the frame to keep it in range of textures
            frameI = mod(frameI,length(A.gratTex(A.side,A.plaid+1).a)); 
