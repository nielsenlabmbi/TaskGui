...
%
% Created by: Kristina Nielsen       Last modified: 250814
...
% Load images - we're doing this here because stimuli cannot be changed
% between trials, so it can be done here

for i = 1:length(S.faceID)
    img=imread([S.basename S.faceID{i} '.tif']);
    img=mean(img,3);
    if i==2
        img=fliplr(img);
    end
    A.imgTex(i) = Screen('MakeTexture',A.window,img);
end
