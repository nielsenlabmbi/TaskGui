...
% LAST TRIAL TYPE, 0 random, 1 pseudorandom, 2 blocking, 3 pseudorandom
% with repeat for incorrect trials
% If new output file, assume the last trial type was random
if A.newOutput
    A.runType = 0; 
    A.newOutput = 0; 
    A.correctResp = 1; %%%change KN
    A.repeatNr = 0; %%%change KN
end
...
switch P.runType
    ...
    case 3 %%%change KN
        %if last trial was incorrect, repeat (up to a certain limit)
        if A.correctResp==0 && A.repeatNr<P.repeatLength
            A.repeatNr=A.repeatNr+1;
            A.stimSide = S.trialsList(A.listPerm(A.listIndex),1);
            P.lineColor = S.trialsList(A.listPerm(A.listIndex),2);
            P.distTheta = S.trialsList(A.listPerm(A.listIndex),3);
            P.stimTheta = S.trialsList(A.listPerm(A.listIndex),4);
        else
            A.repeatNr=0;
            % If last trial was not pseudo random, set up a trial list
            if ~(A.runType == 1)
