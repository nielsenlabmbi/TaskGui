function [A handles] = DotsRun(S,P,A,hObject)
...
state = 0;
% Assume animal will be correct, regular intertrial interval
A.correct = 1; A.durITI = P.durITI; 
% frame counter
frameI = 0;
% Update handles
...
while state < 5 && ~handles.abortTask
    ...
    if state == 4 
        if currentTime > A.responseTime + P.durTone + A.durITI
            state = 5;
        end
    end
    ...
    if currentTime > lastFrameTime + frameTimeStep - 0.006
        ...
        if any(state == [1 2])
            frameI = mod(frameI,size(A.x,2))+1;
            Screen('DrawDots',A.overlay,[A.x(:,frameI)';A.y(:,frameI)'],A.dotSize,1,A.apPos,2);
            %Screen('gluDisk',A.overlay, 1 ,x+dx,y+dy,A.dotSize);
        end
