function [A handles] = FormSegRun2(S,P,A,hObject)

% function A = FormSegRun2(S,P,A)
%
...
while state < 3 && ~handles.abortTask
    ...
    if currentTime > lastFrameTime + frameTimeStep - 0.006
        ...
        switch state
            case 1
                Screen('DrawTextures',A.window,A.distTex,[],A.rectRand2,A.thetaRand2);
                Screen('DrawTextures',A.window,A.distTex,[],A.rectBkgd,A.thetaBkgd);
                Screen('DrawTextures',A.window,A.lineTex,[],A.rectForm1,A.thetaForm1);
            case 2
                Screen('DrawTextures',A.window,A.distTex,[],A.rectRand1,A.thetaRand1);
                Screen('DrawTextures',A.window,A.distTex,[],A.rectBkgd,A.thetaBkgd);
                Screen('DrawTextures',A.window,A.lineTex,[],A.rectForm2,A.thetaForm2);
        end
...
if ~handles.abortTask
    ...
    WaitSecs(P.durTone);
    WaitSecs(P.durITI);   % INTER TRIAL INTERVAL, MAKE THIS A PARAMETER
end

% CLEAR SCREEN
Screen('Flip',A.window);

% GIVE A TIME OUT IF NOT FIRST TRY
if ~A.firstTry, WaitSecs(P.durTimeOut); end
