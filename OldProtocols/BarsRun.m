function [A handles] = BarsRun(S,P,A,hObject)
...
%
% Created by: Sam Nummela       Last modified: 200813
...
while state < 5 && ~handles.abortTask
    ...
    if state == 1 && brokenBeams(2) && A.firstTry
        ...
        % END THE TRIAL NOW IF THE ONE TRY FLAG IS SET TO ONE
        if P.oneTry==1 %%%change KN
            state = 3; 
            A.finishTime = A.firstTime;
            timedel=P.durTimeOut; %%%change KN
        end
    end
    ...
    if state == 4
        if lastFrameTime > A.finishTime + P.durTone + timedel + P.durITI
            state = 5;
        end
    end
